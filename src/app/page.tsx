...
// Pricing Plan data structure (from billing page)
interface PricingPlan {
  title: string;
  price: string;
  description: string;
  features: string[];
  buttonText: string; // Original button text, can be used or overridden
  buttonVariant: VariantProps<typeof buttonVariants>["variant"];
  isPopular?: boolean;
  savePercentage?: string;
  priceId: string; // Kept for consistency, though not directly used for checkout here
}
...
function LandingPricingCard({
  ...
  return (
    ...
    >
      ...
      <CardFooter>
        <Link
          href={user ? "/dashboard/billing" : "/sign-in"}
          className="w-full"
        >
      ...
      >
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {plans.map((plan) => (
              <LandingPricingCard key={plan.title} plan={plan} user={user} />
            ))}
          </div>
